...
export default {
  ...
  methods: {
    ...
    initManager () {
      // 加载器公共事件
      let managerEvent = anyManager => {
        anyManager.manager.onStart = (url, itemsLoaded, itemsTotal) => {
          console.log('onStart')
          anyManager.manager.url = url
        }
        anyManager.manager.onLoad = () => {
          this.resetThreeTipsFun(`${this.tips}\n${anyManager.manager.url} 载入完成!`)
        }
        anyManager.manager.onProgress = (url, itemsLoaded, itemsTotal) => {
          console.log('onProgress')
          this.resetThreeTipsFun(`${this.tips}\n正在请求：${url}  ${parseInt(itemsLoaded / itemsTotal) * 100}%`)
        }
        anyManager.manager.onError = url => {
          anyManager.manager.message = `${url} 载入失败`
        }
      }
      // 初始加载器
      this.managerBox = {
        OBJManager: {manager: new THREE.LoadingManager(), url: ''},
        MTLManager: {manager: new THREE.LoadingManager(), url: ''}
      }
      managerEvent(this.managerBox.OBJManager)
      managerEvent(this.managerBox.MTLManager)
    },
    // 初始加载器
    initLoader () {
      let knifeOBJLoader = new OBJLoader(this.managerBox.OBJManager.manager)
      let knifeMTLLoader = new MTLLoader(this.managerBox.MTLManager.manager)// 材质文件加载器
      knifeMTLLoader.load('/static/three/obj/knife/knife.mtl', material => {
        knifeOBJLoader.setMaterials(material)
        knifeOBJLoader.load('/static/three/obj/knife/knife.obj', mesh => {
    ...
    clearCache () {
      ...
      this.clearObjCache(this.objBox.knife)
    },
