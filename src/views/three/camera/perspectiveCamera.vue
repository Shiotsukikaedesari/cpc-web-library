...
export default {
  ...
  methods: {
    ...
    initGui () {
      ...
      this.gui.add(this.guiParam, 'fov', 0, 180, 1).onChange(data => {
        this.camera.fov = data
        this.camera.updateProjectionMatrix()
      })
      this.gui.add(this.guiParam, 'aspect', 0, 2, 0.1).onChange(data => {
        this.camera.aspect = data
        this.camera.updateProjectionMatrix()
      })
      this.gui.add(this.guiParam, 'near', 0, 1000, 10).onChange(data => {
        this.camera.near = data
        this.camera.updateProjectionMatrix()
      })
      this.gui.add(this.guiParam, 'far', 1000, 2000, 10).onChange(data => {
  ...
  created () {
    ...
    // github链接
    this.resetThreeLinkFun('camera/perspectiveCamera.vue')
  },
  mounted () {
    this.init()
  },
