...
export default {
  ...
  methods: {
    ...
    initLoader () {
      ...
      // alpha通道纹理
      new THREE.TextureLoader(this.managerBox.textureManager).load(
        '/static/three/texture/alpha.jpg',
        texture => {
          ...
          component.objBox.lightSphereOut.customDistanceMaterial = new THREE.MeshDistanceMaterial({
            alphaMap: texture,
            alphaTest: component.objBox.lightSphereOut.material.alphaTest
          })
        }
      )
      this.mapBox = {
    ...
    initObj () {
      ...
      // 光球辅助
      geometry = new THREE.SphereBufferGeometry(5, 12, 12, 8)
      material = new THREE.MeshBasicMaterial({
        side: THREE.DoubleSide
      })
      material.color.copy(this.lightBox.pointLight.color).multiplyScalar(this.lightBox.pointLight.intensity)
      ...
      this.objBox.lightSphereOut = new THREE.Mesh(geometry, material)
      this.objBox.lightSphereOut.castShadow = true
      this.objBox.lightSphereOut.receiveShadow = true
      this.lightBox.pointLight.add(this.objBox.lightSphereOut)
      // 球
      geometry = new THREE.SphereBufferGeometry(25, 12, 12, 8)
      material = new THREE.MeshPhongMaterial({
      ...
      // 方体
      geometry = new THREE.BoxBufferGeometry(50, 50, 50, 1, 1)
      ...
      // 圆环扭曲几何
      geometry = new THREE.TorusKnotBufferGeometry(16, 4, 70, 12, 2, 3)
    ...
    clearCache () {
      ...
      this.clearObjCache(this.objBox.lightSphere)
      this.clearObjCache(this.objBox.lightSphereOut)
      // 清空纹理
      this.mapBox.envMap.dispose()
      this.mapBox.map.dispose()
      this.mapBox.alphaMap.dispose()
      // 页面销毁xhr
