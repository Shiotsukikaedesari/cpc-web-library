...
export default {
  ...
  methods: {
    ...
    // 初始渲染器
    initRender () {
      this.renderer = new THREE.WebGLRenderer({antialias: true, alpha: true, logarithmicDepthBuffer: true}) // 渲染器
    ...
    initLight () {
      this.lightBox = {
        pointLight: new THREE.PointLight('rgb(150, 150, 150)', 2, 300, 0.5) // 点光
      }
    ...
    // 初始相机
    initCamera () {
      this.camera = new THREE.PerspectiveCamera(45, window.innerWidth / window.innerHeight, 50, 1000) // 相机
    ...
    initGui () {
      ...
      cameraSetting.add(this.guiParam, 'cameraNear', 1, 1500, 1)
        .onChange(data => {
          this.camera.near = data
          this.camera.updateProjectionMatrix() // 调用才会生效
        })
      cameraSetting.add(this.guiParam, 'cameraFar', 1, 1500, 1)
        .onChange(data => {
  ...
  created () {
    ...
    // github链接
    this.resetThreeLinkFun('material/meshDepthMaterial.vue')
  },
  mounted () {
    this.init()
  },
